...
async function setAuthor() {
  ...
  try {
    await microblog.set_name(otp, text);
    textarea.value = "";
  } catch (err) {
    console.log(err);
...
async function load_posts() {
  ...
  let posts = await microblog.posts();
  ...
  for (var i = 0; i < posts.length; i++) {
    ...
    post.innerText = posts[i].author + "=> [" + posts[i].content + "] @ " + posts[i].time;
    console.log(posts[i]);
...
async function load_timeline() {
  ...
  let timeline = await microblog.timeline();
